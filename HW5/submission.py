...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        # BEGIN_YOUR_CODE 
        """
        Update the belief of all tiles by the following steps:
        1. Calculate the distance between the tile and the agent
        2. Get the value as Emission Probability at the observedDist in the pdf with mean value dist 
           and the const standard deviation
        3. Set the new probability of the tile as original probability * Emission Probability
        4. Normorlize the belief
        """
    ...
    def elapseTime(self) -> None:
        ...
        # BEGIN_YOUR_CODE 
        """ 
        Create a new belief based on a transition model by the following steps
        1. Create a new belief with value 0
        2. Set the new probability as old probability * transition probability
        3. Normorlize the belief
        """
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        # BEGIN_YOUR_CODE
        """ 
        1. Create a new reweighted particle as a dict() by the steps in part1.
        2. Then resample it by using the weighted random distribution to determine
           the location store the particle.
        3. Finally, update the belief.
        """
        reweightParticle = collections.defaultdict()
        for row, col in self.particles:
            dist = math.dist((util.colToX(col), util.rowToY(row)), (agentX, agentY))
            reweightParticle[(row, col)] = self.particles[(row, col)] * util.pdf(dist, Const.SONAR_STD, observedDist)

        resampleParticle = collections.Counter()
        for _ in range(self.NUM_PARTICLES):
            particle = util.weightedRandomChoice(reweightParticle)
            resampleParticle[particle] += 1 
        self.particles = resampleParticle
        # END_YOUR_CODE
    ...
    def elapseTime(self) -> None:
        # BEGIN_YOUR_CODE
        """ 
        Update the belief by using the weighted random distribution to determine 
        the location store the particle.
        """
